...
# Join texas_with_income with houses_blackout_centroid to get census tracts with a blackout
census_w_blackout <- st_join(texas_with_income, houses_blackout_centroid, join = st_intersects, by = "GEOID_Data")

# Find which census tracts in texas_with_income are not in the houses_blackout_centroid to get census tracts without a blackout

```

```{r}
ggplot(data = census_w_blackout, aes(x = median_income.x)) +
...
I have not been able to figure out how to filter the census data to only the tracts that did not experience a blackout. However, if I could, I would create another histogram like this one to see the shift in distribution. For the tracts that did experience a blackout, it seems that the peak of our histogram, or our most common median income was about $50,000. 
