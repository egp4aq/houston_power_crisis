...
```{r}
# Reclassify the diff_rast, where a drop of more than 200 = blackout

# Create reclassification matrix
# rcl <- matrix(c(200, Inf, 1,
#                 -Inf, 200, NA),
#               ncol = 3, byrow = TRUE)
# # Reclassify raster
# diff_reclass <- classify(diff_rast, rcl)
```
...
bounding_box <- st_bbox(c(xmin = -96.5, xmax = -95.4, ymin = 29, ymax = 30.5)) %>%
  st_as_sfc() %>%
  st_as_sf() %>%
  st_set_crs(st_crs(blackout_mask)) %>%
...
# Crop the raster
blackout_cropped <- st_intersects(blackout_mask, bounding_box)
```
